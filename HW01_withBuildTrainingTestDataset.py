...
class PCA_with_gamma(object):
    ...
    def calculate_gamma_z_score_data(self, file_path, z_score_file, chunksize, gamma):
        ...
        for chunk in tfr:
            ...
            """
            # compare the result of z_score using gamma with the stand lib 
            ...
            end_time = time.time()
            time_write_csv += (end_time - start_time)
            header = False
        ...
        print("Time for writing z_score data to csv file is:", time_write_csv)
...
def plot_confution_matrix(y_true, y_predict, n_class):
    con_mat = confusion_matrix(np.array(y_true), np.array(y_predict))
...
def PCA_sklearn_whole(file_path, n_component):
    pca_std = decomposition.PCA(n_components=n_component)
    df = pd.read_csv(file_path)
    data_x = df.values[:, 1:-1]
    # z-score the dataset
    scaler = StandardScaler()
    scaler.fit(data_x)
    data_x_z_score_std = scaler.transform(data_x)
    pca_std.fit(data_x_z_score_std)
    return pca_std
...
def main():
    ...
    """
     PCA with Sklearn
    """
    start_pca_std_time = time.time()
    pca_sklearn_whole = PCA_sklearn_whole(file_path, n_component)
    end_pca_std_time = time.time()
    print("PCA with Sklearn {} components, explained variance ratio is: {}".format(n_component,
                                                                                   np.sum(
                                                                                       pca_sklearn_whole.explained_variance_ratio_)))
    print("The cost time running PCA with scikit is:", end_pca_std_time - start_pca_std_time)

    """
     PCA with Gamma
    """
    start_pca_gamma_time = time.time()
    pca = PCA_with_gamma()
    ...
    end_pca_gamma_time = time.time()
    print("PCA with Gamma {} components, explained variance ratio is: {}".format(n_component, percentage))
    print("The cost time running PCA with Gamma is:", end_pca_gamma_time - start_pca_gamma_time)
    ...
    # Build the training dataset and test dataset for Gaussian NB
    train_header = True
    test_header = True
    for chunk in tfr:
        chunk_data_x = chunk.values[:, 1:-1]

        # Apply pca transform to z_socre dataset
        chunk_data_reduction = pca.pca_transform(n_component_eigvects, chunk_data_x)

        # Split the dataset to training and test
        chunk_data_reduction_id = np.c_[chunk.values[:, 0], chunk_data_reduction]
        X_train, X_test, y_train, y_test = train_test_split(chunk_data_reduction_id,
                                                            chunk.values[:, -1],
                                                            test_size=0.4,
                                                            random_state=1)

        # Build the training dataset
        train_data_reduction = np.c_[X_train, y_train]
        train_data_reduction = pd.DataFrame(train_data_reduction, index=None, columns=columns)
        train_data_reduction.to_csv(train_file, index=False, header=train_header, mode='a')
        train_header = False

        # Build the test dataset
        test_data_reduction = np.c_[X_test, y_test]
        test_data_reduction = pd.DataFrame(test_data_reduction, index=None, columns=columns)
        test_data_reduction.to_csv(test_file, index=False, header=test_header, mode='a')
        test_header = False
    ...
    y_train_predict_my = NB_my.predict(mu, var, prior, X_train)
    y_test_predict_my = NB_my.predict(mu, var, prior, X_test)
    ...
    plot_confution_matrix(y_test, y_test_predict_my, 2)
